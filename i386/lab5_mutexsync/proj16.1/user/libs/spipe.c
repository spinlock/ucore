...
size_t
spiperead(spipe_t *p, void *buf, size_t n) {
    size_t ret = 0;
...
try_again:
    ...
    for (; ret < n; ret ++, state->p_rpos ++) {
        ...
        *(uint8_t *)(buf + ret) = p->buf[state->p_rpos % SPIPE_BUFSIZE];
    }
out_unlock:
    sem_post(p->spipe_sem);
out:
...
size_t
spipewrite(spipe_t *p, void *buf, size_t n) {
    size_t ret = 0;
    __spipe_state_t *state = p->state;
try_again:
    if (p->isclosed || sem_wait(p->spipe_sem) != 0) {
    ...
    for (; ret < n; ret ++, state->p_wpos ++) {
        ...
        p->buf[state->p_wpos % SPIPE_BUFSIZE] = *(uint8_t *)(buf + ret);
    }
out_unlock:
    sem_post(p->spipe_sem);
out:
...
static bool
__spipeisclosed_nolock(spipe_t *p, bool read) {
    if (!p->isclosed) {
        if (!p->state->isclosed) {
            return 0;
        }
